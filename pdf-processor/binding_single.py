...
def f(team):
    ...
    if len(team_files) > 100 and team != 'AL' and team != 'NL':
            ...
            for team_file in team_files:
                ...
                if team_file[0:4] == str(year) and 'title' not in team_file:
                    if has_scorecards is False:
                        this_merger.append(f"/Volumes/B_Crom_SSD/binding_books_1/{team}/{year}-{team}-title.pdf")
                        has_scorecards = True

                    this_merger.append(f'/Volumes/B_Crom_SSD/binding_books_1/{team}/{team_file}')

            if has_scorecards:
                this_merger.write(f'/Volumes/B_Crom_SSD/bound_books/{team}/{year}-{team}.pdf')

            this_merger.close()
            this_merger = None
    else:
        this_merger = PdfMerger()
        this_merger.append(f"/Volumes/B_Crom_SSD/binding_books_1/{team}/{team}-title.pdf")
        for team_file in team_files:
            if team_file[0:2] == '._': continue
            if 'title' not in team_file:
                this_merger.append(f'/Volumes/B_Crom_SSD/binding_books_1/{team}/{team_file}')

        this_merger.write(f'/Volumes/B_Crom_SSD/bound_books/{team}/{team}.pdf')
        this_merger.close()
        this_merger = None
