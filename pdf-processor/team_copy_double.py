...
def f(team):
    files = os.listdir("/Volumes/B_Crom_SSD/join-mask-pdf")
    os.mkdir(f'/Volumes/B_Crom_SSD/binding_books_2/{team}')
    team_file_list = [f for f in files if f'-{team}-' in f]
    for team_file in team_file_list:
        shutil.copy(f"/Volumes/B_Crom_SSD/join-mask-pdf/{team_file}",
                    f"/Volumes/B_Crom_SSD/binding_books_2/{team}/")
